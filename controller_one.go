...
func newControllerOne(
	...
	return factory.New().
		...
		ToController("controllerOne", recorder.WithComponentSuffix("controller-one"))
}

func (c *controllerOne) sync(ctx context.Context, _ factory.SyncContext) error {
	configMap, err := c.configMapLister.ConfigMaps("foo").Get("bar")
	if err != nil {
		return err
	}
	configMapCopy := configMap.DeepCopy()
	configMapCopy.Data["controllerOne"] = fmt.Sprintf("alive-%d", c.updateCounter)
